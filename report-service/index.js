...
app.get('/full-report', async (req, res) => {
  try {
    ...
    if (!lastReading.temperature || !lastReading.relativeHumidity || !lastReading.windSpeed) {
...
app.get(`/report/heat-index`, async (req, res) => {
  try {
    ...
    const heatIndex = calcHeatIndex(lastReading.temperatura, lastReading.umidade);
